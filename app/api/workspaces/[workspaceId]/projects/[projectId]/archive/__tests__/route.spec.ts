 ...
 * /api/workspaces/{workspaceId}/projects/{projectId}/archive tests.
 * Mocks `updateTeamProjectFields()` — this suite covers auth, request
 * parsing, and status-code mapping only.
 */
...
import { POST } from "@/app/api/workspaces/[workspaceId]/projects/[projectId]/archive/route";
...
function buildRequest(body?: unknown): NextRequest {
  return new NextRequest(`http://localhost/api/workspaces/${WS_ID}/projects/${PROJECT_ID}/archive`, { method: "POST", ...(body !== undefined ? { body: JSON.stringify(body) } : {}) });
}
...
it("400s on an unexpected field", async () => {
  const { res, json } = await callRoute({ ...VALID_BODY, status: "archived" });
...
it("400s on an invalid expectedUpdateTime", async () => {
  const { res } = await callRoute({ expectedUpdateTime: { seconds: "not-a-number" } });
  expect(res.status).toBe(400);
  expect(mockedUpdateTeamProjectFields).not.toHaveBeenCalled();
});

it("passes {uid, workspaceId, projectId, mutation: {kind:'archive'}, expectedUpdateTime} through", async () => {
...
it("200s and emits project_archived only after a successful update", async () => {
  mockedUpdateTeamProjectFields.mockResolvedValue({
    status: "updated",
    project: { id: PROJECT_ID, workspaceId: WS_ID, name: "P", status: "archived", createdByUserId: UID, createdAt: Timestamp.now(), updatedAt: Timestamp.now() },
    documentUpdateTime: Timestamp.now(),
  ...
  expect(mockedWriteProjectEvent).toHaveBeenCalledWith({ eventType: "project_archived", actorUid: UID, workspaceId: WS_ID, projectId: PROJECT_ID });
});

it("409s on invalid_transition (already archived)", async () => {
...
it("409s on precondition_failed (stale token)", async () => {
...
it("maps unauthorized: insufficient_capability -> 403, everything else -> 404 concealed", async () => {
  ...
  mockedUpdateTeamProjectFields.mockResolvedValue({ status: "unauthorized", reason: "membership_not_found" });
