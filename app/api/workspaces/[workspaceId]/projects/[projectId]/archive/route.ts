/**
 * Team Project Backend, Phase 8C-A — `POST /api/workspaces/{workspaceId}/projects/{projectId}/archive`.
 * Mirrors the Personal archive route's dedicated-action-endpoint shape;
 * the lifecycle-transition precondition (`status === "active"`) is
 * validated INSIDE `updateTeamProjectFields()`'s transaction, using the
 * Project state read through that same transaction — not at this route
 * layer, unlike the Personal route (see `lib/firestore/teamProjects.ts`
 * for why: Team authorization is transaction-scoped, so the Project read
 * that would otherwise happen here happens inside the transaction instead).
 */
...
async function getUid(req: NextRequest): Promise<string | NextResponse> {
  ...
  logIdentityResolutionFailure({ route: "POST /api/workspaces/[workspaceId]/projects/[projectId]/archive", method: "POST", failureCategory: identity.reason });
...
export async function POST(req: NextRequest, { params }: { params: { workspaceId: string; projectId: string } }) {
  ...
  switch (updateResult.status) {
    case "updated":
      await writeProjectEvent({ eventType: "project_archived", actorUid: uid, workspaceId, projectId });
