...
- Five route groups touch run data: **Personal/owner** (`/api/user/**` — history, detail, governance, review-history, decision, export×3, reviews inbox, run-governance, synthesize-panel), **Team** (`/api/teams/**` — 10 routes, entirely `teamRuns`-projection-mediated, never read `run.userId`), **Governance** (`/api/governance/**` — its own visibility model, `governanceReviewerUid`/`governanceReviewerFor`), **Admin** (`/api/admin/**` — no ownership scoping at all; tiering was `admin: true` claim only AT THE TIME OF THIS AUDIT, superseded by the ADMIN_PORTAL / SYSTEM_ADMIN / GOVERNANCE_ADMIN split — see `docs/operations/admin-authority-tiers.md`), and roughly 20 non-route `lib/` functions (governance panel/vote/history CRUD, export record CRUD, repair scripts) that read/write `runs/{runId}` but are not independently authorization-gated — they're called from already-authorized routes.
...
| E | `requireAdminApiAccess`/`requireAdmin` — `admin: true` claim only, zero ownership scoping *(historical: both are now deprecated aliases, and `requireAdminApiAccess` is the ADMIN_PORTAL guard which `ADMIN_EMAILS` satisfies without a claim — see `docs/operations/admin-authority-tiers.md`)* | `/api/admin/**` |
