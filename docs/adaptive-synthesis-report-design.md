...
- **8 schemas** (`contested_empirical`, `legal_regulatory`, `financial_valuation`, `procedural`, `medical_health`, `forecast_speculative`, `creative_generative`, `generic` — 8 of the registry's "original 9," `factual_lookup` excluded, see next bullet) — **do** have their own dedicated per-schema view components (`ConsensusMapView`, `RuleApplicationView`, `MetricsGridView`, `StepDiffView`, `EvidenceTiersView`, `ScenarioTreeView`, `GalleryView`), but those views are only reachable as the **"Compare" tab**, buried behind three tabs (List / Compare / Synthesis) where the default landing tab is List and the Synthesis tab is the generic `AdaptiveSynthesisReportView` — "Unified Answer," "Trust Summary," "Panel Verdict Card" — regardless of schema. **A `forecast_speculative` question already has a real `ScenarioTreeView` with probability/leading-indicators built — it's just not what the user sees first.**
- **1 schema** (`factual_lookup`, the 9th of the registry's "original 9") already does almost exactly what this brief asks for its category: `DirectAnswerCard` is a lightweight, answer-first, non-generic renderer, explicitly built to avoid "the full synthesis-report shell."
- **1 schema** (`graceful_limitation`) is neither of the above — verified by code read (`routeClassifiedQuery()` returns `kind: "unanswerable"` for it, which fails `AdaptivePanelResponse.tsx`'s `routing.kind !== "active"` check and returns `LimitationNotice` before any `schema.renderHint` branching or model panel run happens at all). It never reaches the tab shell OR a standalone view — it's a third, separate bucket: a deterministic no-model-run message, not a report. Earlier drafting of this document miscounted it as a 9th tab-shell schema; corrected here.

**This means the module-composer architecture the brief proposes is not a net-new pattern to invent — it's the generalization of a pattern 10 of 19 active schemas already follow, applied to the other 8 report-producing schemas (plus `factual_lookup`'s own precedent generalized as the "keep it short" case for genuinely simple answers; `graceful_limitation`'s 1 schema is out of scope for this document entirely — it has no report to restructure).** The real work is: (1) give those 8 schemas the same "standalone, question-type-specific, no generic shell" treatment their Milestone-2 siblings already have, and (2) build the shared summary-bar/tab/progressive-disclosure chrome around the 18 report-producing schemas consistently, so a user can't tell which "generation" of the schema system produced their answer.
...
| 2. Question-type synthesis | Every schema's own result type (`ComparisonMatrixResult`, `DecisionSupportResult`, `ChecklistTaxonomyResult`, `CausalExplanationResult`, etc.) | Mix of existing standalone views (10/19 schemas), the generic shell (8/19), and `graceful_limitation` (1/19, no report at all — see §1) |
...
| Status | `GovernanceRecordV1.humanReview.status`, using the **already-resolved 8-status model** from `docs/adaptive-research-export-design.md` §4.1 (including the Unreviewed-in-queue / Not-reviewed-no-review-configured split — renamed from an earlier "pending" label to avoid colliding with the raw, currently-unreachable `humanReview.status: "pending"` enum value) | Reuse verbatim — this is the exact status computation the Export design already specified; building it here fulfills that document's own Phase 1 |
