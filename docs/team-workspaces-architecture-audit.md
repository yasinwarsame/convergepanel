...
- **[SUPERSEDED — see `docs/operations/admin-authority-tiers.md`.** `requireAdminApiAccess` is now a deprecated alias for the ADMIN_PORTAL guard, which a verified, enabled `ADMIN_EMAILS` member satisfies with no claim; `/api/admin/runs/[runId]` PATCH and DELETE are SYSTEM_ADMIN since 2026-09-07; and CLAUDE.md's auth section was rewritten. Retained as a record of the state at audit time.]** **Admin API auth was not migrated** to `resolveRequestIdentity` — `requireAdminApiAccess()` (`lib/firebase/auth-helpers.ts:172-208`) still uses the older, independent cookie-or-bearer logic. Not directly relevant to workspaces, but worth fixing opportunistically since a new auth surface will touch this file.
