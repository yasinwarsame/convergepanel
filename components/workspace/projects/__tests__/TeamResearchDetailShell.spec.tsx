...
describe("Team chrome (§N/§AA)", () => {
  ...
  it("the ready Project chrome is identical to the pre-R3 server page chrome (breadcrumb, heading, nav)", async () => {
    ...
    // TEAM-VERIFICATION-PARITY-R5-I2 added the "Videos" nav destination between
    // Claims and Members; that one entry is the only addition to this capture.
    // WORKSPACE-NAV-H1 updated ONLY the WorkspaceNav className strings in this
    // constant (narrow-viewport containment + inset focus ring). Every href,
    // label, order, element type and aria-current below is unchanged, which is
    // what this capture exists to protect.
