/**
 * Team Workspace Core Foundation, Phase 8B — shared sanitized error
 * mappings for `POST /api/user/team-workspaces` and
 * `POST /api/user/team-workspaces/[workspaceId]/transfer-ownership`.
 * Structural mirror of `lib/projects/projectErrorResponse.ts`. Never
 * forwards a raw Firestore error, and never distinguishes, in a single
 * response, which of several internal checks specifically failed when
 * doing so could help an attacker enumerate another workspace's/
 * membership's state.
 */
