/**
 * Phase 5C — dark UI rollout for the Personal Workspace shell.
 *
 * Structural mirror of `lib/workspaces/personalRunWorkspaceWriteCanary.ts`
 * (Phase 3A's account-scoped write canary) — same exact-uid-only matching,
 * same trim/dedupe/max-10 rules, same `getPersonalWorkspaceId().ok` reuse
 * for uid validity, same global-wins-over-malformed-canary precedence.
 * This module decides UI PRESENTATION only — it has no dependency on, and
 * no effect on, `WORKSPACES_ENABLED` / `PERSONAL_WORKSPACE_PROVISIONING_ENABLED`
 * / `PERSONAL_RUN_WORKSPACE_WRITES_ENABLED` / their canary. Phase 5B's read
 * APIs never call this module and remain reachable to any authenticated
 * user regardless of what this resolver decides.
 */
 ...
 * `MAX_WORKSPACE_UI_CANARY_UIDS` distinct entries) fails the WHOLE list,
 * never partially — see `resolvePersonalWorkspaceUiMode()` for what a
 * `false` result here actually does to eligibility.
 */
export function parseWorkspaceUiCanaryUids(raw: string | undefined): WorkspaceUiCanaryParseResult {
  ...
  if (uids.size > MAX_WORKSPACE_UI_CANARY_UIDS) {
 ...
 * call site (neither `/api/user/usage` nor `/workspace`'s route gate).
