...
describe("buildRunReadPayload — envelope interpretation", () => {
  it("emits the frozen key order and every persisted envelope for an owner", async () => {
    ...
    const r = resolver();
    const payload = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data, viewerRole: "owner", resolveReviewRouting: r });
  ...
  it("prefers runDocument.perModel and only falls back to the legacy results[] when rehydration is empty", async () => {
    ...
    const a = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: withBoth, viewerRole: "owner", resolveReviewRouting: resolver() });
    ...
    const b = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: legacyOnly, viewerRole: "owner", resolveReviewRouting: resolver() });
    ...
    const c = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: neither, viewerRole: "owner", resolveReviewRouting: resolver() });
  ...
  it("synthesisCache requires BOTH a report and schemaVersion 1; synthesizedBy defaults to 'cached'; consensus defaults to null", async () => {
    const noVersion = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ schemaVersion: 2 }), viewerRole: "owner", resolveReviewRouting: resolver() });
    expect(noVersion.synthesisCache).toBeNull();
    const noReport = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ synthesizedStructuredReport: undefined }), viewerRole: "owner", resolveReviewRouting: resolver() });
    expect(noReport.synthesisCache).toBeNull();
    const defaults = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ synthesizedBy: undefined, synthesisConsensusSummary: undefined }), viewerRole: "owner", resolveReviewRouting: resolver() });
  ...
  it("governanceStatus accepts only the three org statuses; governance is null when teamGovernance carries no signal", async () => {
    const bogus = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ governanceStatus: "weird", teamGovernance: { policyFlags: [], blocked: false, blockMessage: "", governanceReviewRequired: false } }), viewerRole: "owner", resolveReviewRouting: resolver() });
    expect(bogus.governanceStatus).toBeNull();
    expect(bogus.governance).toBeNull();
    const blocked = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ governanceStatus: "blocked", teamGovernance: { blocked: true, blockMessage: "Stop" } }), viewerRole: "owner", resolveReviewRouting: resolver() });
  ...
  it("adaptive: absent / unsupported_version / malformed are distinct non-error states with null output, null humanReview and reviewRouting 'unknown'", async () => {
    for (const [raw, reason] of [
      ...
      const r = resolver();
      const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ adaptiveOutput: raw }), viewerRole: "owner", resolveReviewRouting: r });
  ...
  it("legacyAdaptive is parsed independently of adaptive — a legacy-only run is adaptive 'absent' AND legacyAdaptive 'valid'", async () => {
    const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ adaptiveOutput: undefined, governanceRecord: undefined }), viewerRole: "owner", resolveReviewRouting: resolver() });
    expect(p.adaptive.status).toBe("absent");
    expect(p.legacyAdaptive.status).toBe("valid");
    const malformed = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ legacyAdaptiveOutput: { version: 1, schemaId: "procedural" } }), viewerRole: "owner", resolveReviewRouting: resolver() });
  ...
  it("humanReview carries status/conditions/decidedVia only — never reviewer id, name or comment", async () => {
    const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ governanceRecord: governanceRecord("approved_with_conditions") }), viewerRole: "owner", resolveReviewRouting: resolver() });
  ...
  it("a malformed governanceRecord beside a valid adaptive envelope yields humanReview null and no review-routing I/O", async () => {
    const r = resolver();
    const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ governanceRecord: { version: 1 } }), viewerRole: "owner", resolveReviewRouting: r });
...
describe("buildRunReadPayload — review routing seam", () => {
  it.each(["unreviewed", "pending"])("status %s → resolver called once with runId, the RUN OWNER uid and the requestId; its value is surfaced", async (status) => {
    ...
    const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ governanceRecord: governanceRecord(status) }), viewerRole: "team_member", requestId: "req-1", resolveReviewRouting: r });
  ...
  it.each(["approved", "approved_with_conditions", "changes_requested", "rejected"])("decided status %s → resolver NOT called; reviewRouting stays 'unknown'", async (status) => {
    const r = resolver();
    const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ governanceRecord: governanceRecord(status) }), viewerRole: "owner", resolveReviewRouting: r });
    expect(r).not.toHaveBeenCalled();
    if (p.adaptive.status !== "valid") throw new Error("expected valid adaptive");
    expect(p.adaptive.reviewRouting).toBe("unknown");
  });
...
describe("buildRunReadPayload — deep research claim ids and redaction", () => {
  it("attaches deterministic claimIds to deep_research findings at response time without mutating the input document", async () => {
    ...
    const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data, viewerRole: "owner", resolveReviewRouting: resolver() });
    ...
    const again = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData(), viewerRole: "owner", resolveReviewRouting: resolver() });
  ...
  it("does not attach claimIds to a non-deep_research envelope", async () => {
    const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData({ adaptiveOutput: comparisonMatrixAdaptiveOutput(), governanceRecord: governanceRecord("approved", { schemaId: "comparison_matrix", answerShape: "comparison_grid" }) }), viewerRole: "owner", resolveReviewRouting: resolver() });
    if (p.adaptive.status !== "valid") throw new Error("expected valid adaptive");
  ...
  it.each(["personal_reviewer", "team_reviewer"] as const)("%s loses tokenUsage and latencyMs on every row, same length and other fields intact", async (role) => {
    const p = await buildRunReadPayload({ runId: FIXTURE_RUN_ID, data: fullTeamRunData(), viewerRole: role, resolveReviewRouting: resolver() });
